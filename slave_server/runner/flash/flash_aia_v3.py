...
passwd = _get_passwd()
os.system("echo '"+passwd+"'|sudo -S chmod 777 /dev/sdc1")

class Aia_flash(threading.Thread):

    def __init__(self, build, build_type, url, target, **kwargs):
        self.serial = "DW1724778700001"
        self.relay_USB = "/dev/ttyACM0" #self.get_mapping_dev("relayCard")
        self.udisk = "/dev/sdc1" #self.get_mapping_dev("U_Disk")
    ...
    def format_udisk(self):
        print "format udisk"
        status_output = commands.getstatusoutput("mkfs.fat {}".format(self.udisk))
	...
	self.relay_ser.write(chr(104))
	time.sleep(0.3)
	self.relay_ser.write(chr(114))
    ...
    def check_flash_result(self):
        flash_image_version = self.image_path.split("-")[-1]
