...
#    return self._GetEmptyResults()
  ...
  @staticmethod
  def _GetEachFPS(data):
    deltas = [t2 - t1 for t1, t2 in zip(data, data[1:])]
    return ( [1 / delta for delta in deltas])
  ...
  def _CalculateResults(refresh_period, timestamps, result_suffix):
    ...
    all_fps = SurfaceStatsCollector._GetEachFPS(timestamps)
    print all_fps
    print "averageFPS = ", sum(all_fps) / len(all_fps)
  ...
  @staticmethod
  def _ProcessTimestamps(refresh_period, timestamps, ts_number):
    results = []
    sliced = timestamps[ -((int)(ts_number)) : ]
    results += SurfaceStatsCollector._CalculateResults(
          refresh_period, sliced, '')
    return results
...
#    self._results += self._ProcessTimestamps(refresh_period, timestamps, 10)
  ...
  def _ClearSurfaceFlingerLatencyData(self):
    ...
    results = self._adb.RunShellCommand(
        'dumpsys SurfaceFlinger --latency-clear '+self._win)
  ...
  def _GetSurfaceFlingerFrameData(self):
    ...
    results = self._adb.RunShellCommand(
        'dumpsys SurfaceFlinger --latency '+self._win,
