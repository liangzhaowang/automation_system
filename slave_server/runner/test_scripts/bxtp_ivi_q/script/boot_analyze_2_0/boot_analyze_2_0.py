...
class Logs_Parse:
    def __init__(self, log_path, config):
        ...
        self.config = config
        self.tmplist = []
        self.need_adjust_time = False
        self.ms_delta = 0
        self.date_delta = datetime.timedelta(seconds=0)
        self.adjusted_logs = []
        self.init_data = self.get_data()

        #backup adjusted logs for debug
        if len(self.adjusted_logs) > 0:
            f = open(os.path.join(log_path, "adjusted_logcat.log"), "w")
            for _ in self.adjusted_logs:
                f.write(_)
            f.close()
            ...
            if os.path.exists(log_file) and os.path.getsize(log_file) != 0 and not re.match('Could not open memory file',log_doc):
                try:
                    ...
                    for i in self.config['log'][key_name]['tag']:
                        #check whether system time was changed by alarmService
                        if i['name']=="start_alarmService":
                            for line in lines:
                                if re.search(i['filter'], line):
                                    self.get_delta(line)
                                    self.need_adjust_time = True
                                    break

                    for i in self.config['log'][key_name]['tag']:
                        for line in lines:
                            time_filter = -1
                            newline = self.check_time_jump(line)
                            if re.search(i['filter'], newline):
                                time_filter = self.LogTime(self.config['log'][key_name]['config']['time_pattern'], newline, self.config['log'][key_name]['config']['time_index'])
    ...
    def get_delta(self, log):
        print "log_time was changed:%s" % log
        time_src = re.findall("\d{13}", log)
        if len(time_src) < 2:
            logs("expected log, failed to get delta time!")
            return
        else:
            '''
            log: Current time only 1321009878680, advancing to build time 1556049085000
            that leads a big time jump, need adjust accordingly
            '''
            t_format = "%m-%d %H:%M:%S"
            t1 = time.strftime(t_format, time.localtime(int(time_src[0])//1000))
            t2 = time.strftime(t_format, time.localtime(int(time_src[1])//1000))
            self.date_delta = datetime.datetime.strptime(t2, t_format) - datetime.datetime.strptime(t1, t_format)
            self.ms_delta = int(time_src[0])%1000
            log_date = re.findall("\\d+\-\\d+\ \\d+:\\d+:\\d+.\\d+", log)[0][:-4]
            #base_date used to check if time jumped
            self.base_date = datetime.datetime.strptime(log_date, t_format)
        return

    def check_time_jump(self, buf):
        new_line = buf
        if not self.need_adjust_time:
            return new_line
        else:
            delta = self.date_delta
            t_format = "%m-%d %H:%M:%S"
            log_time = re.findall("\\d+\-\\d+\ \\d+:\\d+:\\d+.\\d+", buf)
            if len(log_time) < 1:
                return buf
            ...
            if len(log_time) > 0:
                new_line = buf
                total_s = (datetime.datetime.strptime(log_time[:-4], t_format) - self.base_date).total_seconds( )
                #if catch big time jump, adjust
                if abs(total_s) > 1000:
                    ms_time = int(log_time[15:]) + int(self.ms_delta)
                    if ms_time > 1000:
                        delta -= datetime.timedelta(seconds=1)
                        ms_time = ms_time % 1000
                    adjust_time = datetime.datetime.strptime(log_time[:-4], t_format) - delta
                    new_line = "%s.%03d%s" % (datetime.datetime.strftime(adjust_time, t_format), int(ms_time), buf[18:])
                    self.adjusted_logs.append(new_line)
            return new_line
