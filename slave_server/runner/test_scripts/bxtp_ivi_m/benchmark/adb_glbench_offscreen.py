...
def get_result():
	alltext = 'adb logcat -d | grep "Application::FinishCurrentTest" >' +os.path.join(CUR_DIR,out_dir,'glbench_offscreen.txt')
	os.popen(alltext)
	...
	if len(text) > 0 :
		...
		#result = text[0].replace('frames','@').strip().strip(')').split()
		print text[0]
		array = text[0].strip('frames ').strip(')').split()
...
def main():
	...
	# comment for debug
	tr.result(result)
	tr.add_attachment(os.path.join(CUR_DIR,out_dir, 'glbench_offscreen.txt'),'glbench_offscreen.txt')
