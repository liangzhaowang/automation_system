...
def get_result():
	result_str = ''
	alltext = 'adb logcat -d | grep -e _logcat.*score -e _logcat.*fps | cut -d ":" -f 4,5 >' +os.path.join(CUR_DIR,out_dir, 'manhattan_onscreen.txt')
	os.popen(alltext)
...
def click_test_onscreen_gfx311():
...
def main():
	...
	tr.add_attachment(os.path.join(CUR_DIR,out_dir, 'manhattan_onscreen.txt'),'manhattan_onscreen.txt')
	tr.save()
