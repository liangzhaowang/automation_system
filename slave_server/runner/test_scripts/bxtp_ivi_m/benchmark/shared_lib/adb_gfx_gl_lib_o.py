...
class gfx:
	...
	def select_item(self,timeout = 5):
		...
		icount = 0
		# app_dev =''
		view_dev =self.mobile_test.device(text="1080p Manhattan Offscreen")
	...
	def collect_result(self):
		try:
			result = ""
			kill_adb_uiautomator_block()
			fps_score = self.mobile_test.get_score_info_data("updated_result_item_subresult",'text',"1")
			kill_adb_uiautomator_block()
			result = re.sub("[()\xc2\xa0Fps]","",fps_score)
			# result =re.search(r".+(\d+).+",fps_score).group(0)
			if result == "":
				return False
			return result
		except Exception,e:
