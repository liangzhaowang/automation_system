...
from exception_operation import app_exception_kill_case
from exception_operation import wait_for_boot
from base_lib.watt_opteration import watt_operate
...
class ui_operation(object):
	...
	def wait_for_complete(self,complete_text,timeout=1800,splittime = 5,ABS=30,skipstep=30,package = ''):
		i = 0
		failed_time = 0
				...
				if app_dev.exists:
					...
					if self.watt_operate.watt_flags:
						...
						try:
							...
							self.logout(self.my_func_name(), "watt_operate error %s."%e)
							return False
					return True
			else:
				i+=skipstep
	...
	def wait_for_complete_by_text(self,complete_text,timeout=1800,ABS=30):
		i = 0
		failed_time = 0
	...
	def click_ui_button_by_resourceIdMatches_to_right_resourceIdMatches(self,test_text,key_code,timeout=10):
		icount = 0
		kill_adb_uiautomator_block_old()
		while icount < timeout:
			unclock_android_o_screen()
	...
	def wait_for_open_spacial_function(self,sp_text = "",resourceIdMatches_txt = "",timeout=600):
		...
		self.logout(self.my_func_name(),"Wait for %s spacial function."%self.app_package)
		for i in range(timeout):
			unclock_android_o_screen()
			if i % 5 == 0:
				kill_adb_uiautomator_block()
	...
	def click_ui_button_by_text(self,key_code="",textcont="",index_nu = "",timeout=10):
		icount = 0
		kill_adb_uiautomator_block_old()
		while icount < timeout:
			unclock_android_o_screen()
	...
	def system_setup(self,tog_airplane = False,
					tog_wifi = False,
					tog_bt = False,
					tog_gps = False,
					tog_display = True
