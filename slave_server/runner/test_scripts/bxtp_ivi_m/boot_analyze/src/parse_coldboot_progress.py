...
class p_node:
    ...
    def aptime_set(self, timestamp, timebase,timegap=0):
        ...
        if not timebase == '':
            ...
            self.seconds = self.seconds - (int(timebase[0]) * 3600 + int(timebase[1]) * 60 + float(timebase[2]))-timegap
...
class bootpgs_parser:
    def __init__(self, needDump, inputFile, outputDir=""):
        self.nodeList = []
        self.outputDir = outputDir
        self.dmesgFile = "dmesg.txt"
    ...
    def initStages(self):
        with open(os.path.join(CUR_DIR, '../config/coldboot_progress.json'), 'r') as f:
            data = json.load(f)
    ...
    def parseLogcat(self):
        ...
        for i in range(len(lines)):
            if lines[i].find('Service com.android.server.AlarmManagerService took')!=-1:
                time1 = lines[i].split()[1].split(':')
            if lines[i].find('Setting time of day')!=-1:
                time2 = lines[i].split()[1].split(':')
            elif lines[i].find('Starting com.android.server.AlarmManagerService')!=-1:
                time2 = lines[i].split()[1].split(':')
            if (time1 and time2):
                timegap = (int(time1[0])*3600+int(time1[1])*60+float(time1[2]))-(int(time2[0])*3600+int(time2[1])*60+float(time2[2]))
        ...
        for l in lines:
            if l.find('Linux version') != -1:
                ...
                for i in range(len(lines)):
                    if lines[i].find('beginning of events')!=-1:
                        timebase = lines[i+1].split()[1]

            for node in self.nodeList:
                if node.filterkey is not None and re.search(node.filterkey, l):
                    if timegap!=0:
                        if l.find('boot_progress_ams_ready')!=-1:
                            timestamp = l.split()[1]
                            node.aptime_set(timestamp, timebase,timegap)
                            node.proc = l.split()[2].split('/')[1].strip()
                            break
                        if l.find('boot_progress_enable_screen')!=-1:
                            timestamp = l.split()[1]
                            node.aptime_set(timestamp, timebase,timegap)
                            node.proc = l.split()[2].split('/')[1].strip()
                            break
                        if l.find('Boot is finished')!=-1:
                            timestamp = l.split()[1]
                            node.aptime_set(timestamp, timebase,timegap)
                            node.proc = l.split()[2].split('/')[1].strip()
                            break
    ...
    def showResult(self):
        ...
        for node in self.nodeList:
            ...
            out.write((node.rank - 1) * ',' + (3 - node.rank) * ('%-s,' % (self.getNameWithFlag(node))) + '%-s,' % (
            self.getNameWithFlag(self.nodeList[nextIdx])) + '%.7f,' % (node.seconds) + '%.7f' % (duration))
...
def parse_coldboot_progress(needDump, inputFile, outdir):
    if not outdir:
        outdir = ''
    outdir = os.path.join(CUR_DIR, "../" + outdir)
    print "outdir== ", outdir
    reload(sys)
    ...
    if needDump:
        os.system("adb root")
        time.sleep(2)
    parser = bootpgs_parser(needDump, inputFile, outdir)
    parser.initStages()
    parser.getLogs()
    parser.parseLogs()
    parser.showResult()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-O', '--outdir', action='store', dest='outdir', help='rename outdir')
    ...
    if args.direct:
        boot_parser = parse_coldboot_progress(True, None, args.outdir)
    elif args.logfile:
        boot_parser = parse_coldboot_progress(False, args.logfile, args.outdir)
