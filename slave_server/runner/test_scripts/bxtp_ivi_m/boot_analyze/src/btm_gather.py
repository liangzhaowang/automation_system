...
def readgrabTime(line):
    nanoseconds = 0
    regex = re.compile("^\[(?P<time0>\d+\.\d+) (?P<time1>\d+\.\d+)\] +(?P<line>.*)$")
    match = regex.match(line)
    if match:
        try:
            nanoseconds = int(match.groupdict().get('time0', '').split('.')[0]) * 1000000 + int(match.groupdict().get('time0', '').split('.')[1])
        except IndexError:
            pass
    if nanoseconds == 0:
        return nanoseconds
    else:
        return nanoseconds - iocTimeOffset
...
def readgrabserialext(inputfile):
    entryList=[]
    global out_dir
    configList = readconfig(inputfile)
    for logline in open(os.path.join(CUR_DIR,'../'+out_dir+'/grabserial.txt')):
        logline = logline.strip('\n').strip('\r').strip(' ')
        text = logline.lstrip('[].1234567890 ')
        for line in configList:
                items = line.split('|')
...
#     timestamp ="0.000000"
#     for line in lines:
#         if "[drm:skl_update_scaler] scaler_user index 0.2:" in line:
#             sobj= re.search(r'[drm:skl_update_scaler] scaler_user index 0.2(?P<time>\d+\.\d+)',line,re.I|re.M)
#             if sobj:
#                 timestamp = sobj.group('time')
#                 if timestamp == "0.000000":
#                     sobjx = re.search(r'[drm:intel_atomic_setup_scalers](?P<time>\d+\.\d+)',line,re.I|re.M)
#                     if sobjx:
...
def get_rvc_read_timestamp():
    ...
    for line in lines:
        if "[drm:skl_update_scaler] scaler_user index 0.2:" in line:
            tmp_times=re.search(r'\[(.+)\] \[drm:skl_update_scaler\] scaler_user index 0.2:', line).group(1).lstrip()
            # print tmp_times
            rvc_timestamp.append(tmp_times)
        else:
            pass
    timestamp = str(float(rvc_timestamp[0]))
    return timestamp
    ...
    for line in entryList:
        ...
        if(rvc_start[2].lstrip().strip()=="RVC" and rvc_start[3].lstrip().strip()=="start"):
            ...
            if timestp == "0.000000":
                ...
                sys.exit()
            rvc_start[3]=" ready"
            rvc_start[4]= str(round(float(timestp)-float(rvc_start[1]),3))
            rvc_start[0] = str(round(float(rvc_start[0])+float(rvc_start[4]),3))
            rvc_start[1] = str(round(float(rvc_start[1])+float(rvc_start[4]),3))
...
def main(argv):
    ...
    #print entryList
    calcOffset()
    grabserial_dicts=readgrabserialext(inputfile)
    entryList = entryList + readAblDump()+grabserial_dicts
