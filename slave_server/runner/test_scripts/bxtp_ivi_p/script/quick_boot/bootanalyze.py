...
def main():
  ...
  args = init_arguments()
  CUI_DIR = os.path.dirname(os.path.abspath(__file__))
  files_dir = tr.result_dir
  if not os.path.exists(files_dir):
    os.mkdir(files_dir)
  
  log=logger("quicklog", files_dir + "/" + "quicklog.txt")
  file_dir = files_dir + "/"
  ...
  for it in range(0, args.iterate):
    ...
    if not processing_data or not boottime_events:
      # Processing error
      log.debug("Failed to collect valid samples for run {0}".format(it))
      continue
    if args.bootchart:
      grab_bootchart(boot_chart_file_name_prefix + "_run_" + str(it),file_dir)
      

    if args.systrace:
      grab_systrace(systrace_file_name_prefix + "_run_" + str(it),file_dir)
  ...
  if args.iterate > 1:
    log.debug("-----------------")
    log.debug("ro.boottime.* after {0} runs".format(args.iterate))
    log.debug('{0:30}: {1:<7} {2:<7} {3}'.format("Event", "Mean", "stddev", "#runs"))


    for item in boottime_points.items():
      num_runs = len(item[1])
      log.debug('{0:30}: {1:<7.5} {2:<7.5} {3} {4}'.format(
    ...
    log.debug("-----------------")
    log.debug("Avg values after {0} runs".format(args.iterate))
    log.debug('{0:30}: {1:<7} {2:<7} {3}'.format("Event", "Mean", "stddev", "#runs"))
    average_with_stddev = []
    ...
    for item in sorted(average_with_stddev, key=lambda entry: entry[1]):
      log.debug('{0:30}: {1:<7.5} {2:<7.5} {3}'.format(
        item[0], item[1], item[2], item[3] if item[3] != args.iterate else ""))


    tmp_list=sorted(average_with_stddev, key=lambda x: x[1])
    start=[]
    duration=[]
    cate=[]
    for tmp_i in range(len(tmp_list)):
      start.append(round(float(tmp_list[tmp_i][1]),3))
      if (tmp_i == len(tmp_list)-1):
        duration.append(0)
      else:
          duration.append(round(float(tmp_list[tmp_i+1][1])-float(tmp_list[tmp_i][1]), 3))
      cate.append(tmp_list[tmp_i][0])
    dic = {'starts': start, 'categories': cate, 'durations': duration}
    # json.dumps(dic)
    tmp_file=open(file_dir+"result.json","w+")
    tmp_file.write(json.dumps(dic))
    tmp_file.close()
    result=0.0
    try:
      i=dic["starts"][-1]
      ...
      log.debug("Generate result.json Failed")
    log.debug("************************************************")
    log.debug("result_time is: "+str(result))
    log.debug("************************************************")
    time.sleep(30)
    run_adb_shell_cmd_as_root('rm /data/bootchart/enabled')

    if os.path.exists(os.path.join(file_dir+boot_chart_file_name_prefix+ "_run_" + str(it-1)+'.tgz')):
      tr.add_attachment(os.path.join(file_dir+boot_chart_file_name_prefix+ "_run_" + str(it-1)+'.tgz'))
    if os.path.exists(os.path.join(file_dir+boot_chart_file_name_prefix+ "_run_" + str(it)+'.tgz')):
      tr.add_attachment(os.path.join(file_dir+boot_chart_file_name_prefix+ "_run_" + str(it)+'.tgz'))
    if os.path.exists(os.path.join(file_dir+boot_chart_file_name_prefix+ "_run_" + str(it-1)+'.png')):
      tr.add_attachment(os.path.join(file_dir+boot_chart_file_name_prefix+ "_run_" + str(it-1)+'.png'))
    if os.path.exists(os.path.join(file_dir+boot_chart_file_name_prefix+ "_run_" + str(it)+'.png')):
      tr.add_attachment(os.path.join(file_dir+boot_chart_file_name_prefix+ "_run_" + str(it)+'.png'))
    tr.add_attachment(os.path.join(file_dir+ "quicklog.txt"))
    tr.add_attachment(os.path.join(file_dir+'result.json'),'stackbar')
    if os.path.exists(os.path.join(file_dir+systrace_file_name_prefix+ "_run_" + str(it-1)+".h5")):
      tr.add_attachment(os.path.join(file_dir+systrace_file_name_prefix+ "_run_" + str(it-1)+".h5"))
    if os.path.exists(os.path.join(file_dir+systrace_file_name_prefix+ "_run_" + str(it)+".h5")):
      tr.add_attachment(os.path.join(file_dir+systrace_file_name_prefix+ "_run_" + str(it)+".h5"))
    if os.path.exists(os.path.join(file_dir+systrace_file_name_prefix+ "_run_" + str(it-1)+"_trace.txt")):
      tr.add_attachment(os.path.join(file_dir+systrace_file_name_prefix+ "_run_" + str(it-1)+"_trace.txt"))
    if os.path.exists(os.path.join(file_dir+systrace_file_name_prefix+ "_run_" + str(it)+"_trace.txt")):
      tr.add_attachment(os.path.join(file_dir+systrace_file_name_prefix+ "_run_" + str(it)+"_trace.txt"))
    tr.result(result)
    tr.save()
    return True
...
def dump_timings_points_summary(msg_header, timing_points, args):
      ...
      log.debug("-----------------")
      log.debug(msg_header + " timing in order, Avg time values after {0} runs".format(args.iterate))
      log.debug('{0:30}: {1:<7} {2:<7} {3}'.format("Event", "Mean", "stddev", "#runs"))
      ...
      log.debug("-----------------")
      log.debug(msg_header + " timing top items, Avg time values after {0} runs".format(args.iterate))
      log.debug('{0:30}: {1:<7} {2:<7} {3}'.format("Event", "Mean", "stddev", "#runs"))
...
def capture_bugreport(bugreport_hint, boot_complete_time,file_dir):
    log.debug("Boot up time too big")
...
def dump_timing_points(msg_header, timing_points):
    log.debug(msg_header + " event timing in time order, key: time")
    for item in timing_points.items():
      log.debug('{0:30}: {1:<7.5}'.format(item[0], item[1]))
    log.debug("-----------------")
    log.debug(msg_header + " event timing top items")
    for item in sorted(timing_points.items(), key=operator.itemgetter(1), reverse = True):
      if item[1] < TIMING_THRESHOLD:
        break
      log.debug('{0:30}: {1:<7.5}'.format(
        item[0], item[1]))
    log.debug("-----------------")
...
def iterate(args, search_events, timings, cfg, error_time, components_to_monitor):
  if args.reboot:
    ...
    # os.system("adb shell su root dmesg > %sdemsg.txt"%file_dir)
  ...
  if not logcat_event_time.get(KERNEL_TIME_KEY):
    log.debug("kernel time not captured in logcat, cannot get time diff")
  ...
  if not dmesg_event_time.get(KERNEL_BOOT_COMPLETE):
    log.debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    log.debug(dmesg_event_time)
    log.debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    log.debug("BootAnimEnd time or BootComplete-kernel not captured in both log" +\
  ...
  log.debug("-----------------")
  log.debug("ro.boottime.*: time")
  for item in boottime_events.items():
    log.debug('{0:30}: {1:<7.5} {2}'.format(item[0], item[1],\
      "*time taken" if item[0].startswith("init.") else ""))
  log.debug("-----------------")
  ...
  for k, v in data_points.iteritems():
    log.debug('{0:30}: {1:<7.5} {2:1} ({3})'.format(
  ...
  if events[LOGCAT_BOOT_COMPLETE] > error_time and not args.ignore:
    capture_bugreport("bootuptoolong", events[LOGCAT_BOOT_COMPLETE],file_dir)
  ...
  for k, v in components_to_monitor.iteritems():
    ...
    if logcat_value_measured and logcat_value_measured > v:
      capture_bugreport(k + "-" + str(logcat_value_measured), events[LOGCAT_BOOT_COMPLETE],file_dir)
      break
    elif kernel_value_measured and kernel_value_measured > v:
      capture_bugreport(k + "-" + str(kernel_value_measured), events[LOGCAT_BOOT_COMPLETE],file_dir)
      break
    elif data_from_data_points and data_from_data_points['value'] * 1000.0 > v:
      capture_bugreport(k + "-" + str(data_from_data_points['value']), events[LOGCAT_BOOT_COMPLETE],file_dir)
      break
  ...
  if args.fs_check:
    ...
    if logcat_events.get("FsStat"):
      ...
      if m:
        fs_stat = m.group(1)
    log.debug('fs_stat:'+ str(fs_stat))

    if fs_stat:
      fs_stat_val = int(fs_stat, 0)
      if (fs_stat_val & ~0x17) != 0:
        capture_bugreport("fs_stat_" + fs_stat, events[LOGCAT_BOOT_COMPLETE],file_dir)
...
def init_arguments():
  ...
  parser.add_argument('-t', '--timings', dest='timings', action='store_true',
                      help='log.debug(individual component times', default=True, )
        ...
        if pid == primary_pid: # old one was secondary:
          ...
          for item in move_to_secondary:
            ...
            if item[0].endswith("-secondary"):
              log.debug("Secondary already exists for event %s  while found new pid %d, primary %d "\
...
def collect_events(search_events, command, timings, stop_events, disable_timing_after_zygote):
  ...
  while True:
    ...
    if time_left <= 0:
      log.debug("timeout waiting for event, continue" + str(time_left))
      break
    read_r = read_poll.poll(time_left * 1000.0)
    if read_r:
        line = process.stdout.readline()
    else:
      log.debug("poll timeout waiting for event, continue " + str(time_left))
      break
    if not data_available:
      log.debug("Collecting data samples from '%s'. Please wait...\n" % command)
      data_available = True
    ...
    if event:
      ...
      if event in stop_events:
        ...
        log.debug("remaining stop_events:".join(stop_events))
...
def extract_time(events, pattern, date_transform_function):
  ...
  for event, data in events.iteritems():
    ...
    if time:
      ...
      log.debug("Failed to find time for event: "+event+data)
  return result
...
def do_reboot(serial, use_adb_reboot):
  ...
  if use_adb_reboot:
    log.debug('Rebooting the device using adb reboot')
    run_adb_cmd('reboot')
  else:
    log.debug("reboot 648")
    log.debug('Rebooting the device using svc power reboot')
...
def reboot(serial, use_stressfs, permissive, use_adb_reboot):
  if use_stressfs:
    log.debug('Starting write to data partition')
...
def logcat_time_func(offset_year):
  def f(date_str):
    ndate = datetime.datetime.strptime(str(offset_year) + '-' + date_str, '%Y-%m-%d %H:%M:%S.%f')
...
def grab_bootchart(boot_chart_file_name,file_dir):
  passwd = _get_passwd()
  subprocess.call("echo "+passwd+"|sudo ./runner/test_scripts/bxtp_ivi_o/script/quick_boot/lib/grab-bootchart.sh", shell=True)
  os.system("cp ./bootchart.png " +file_dir+boot_chart_file_name+".png")
  log.debug("Saving boot chart as " + boot_chart_file_name + ".tgz")
  subprocess.call('cp /tmp/android-bootchart/bootchart.tgz ' + file_dir + boot_chart_file_name + '.tgz', shell=True)

def grab_systrace(systrace_file_name,file_dir):
  passwd = _get_passwd()
  trace_file = file_dir + '/' + systrace_file_name + "_trace.txt"
  print trace_file
  with open(trace_file, 'w') as f:
    f.write("TRACE:\n")
  run_adb_shell_cmd_as_root("cat /d/tracing/trace >> " + trace_file)
  html_file = file_dir + "/" + systrace_file_name + ".h5"
  subprocess.call("echo "+passwd+"|sudo ./runner/test_scripts/bxtp_ivi_o/script/quick_boot/lib/chromium-trace/systrace.py --from-file=" + trace_file + " -o " + html_file, shell=True)
