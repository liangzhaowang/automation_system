...
options:
	...
	-P, --acrn_pwd=<pwd>        Acrn pwassword for test
	-u, --acrn_user=<user>       Acrn username for test
	-j, --journal=<journal>       scp path for journal log
...
def grab(arglist, outputfd=sys.stdout):
	...
	try:
		opts, args = getopt.getopt(arglist,
                        "hli:d:b:B:w:p:s:xrfc:tTm:e:o:vVq:SP:j:u:", [
				"help",
				"launchtime",
				"instantpat=",
				"device=",
	...
	for opt, arg in opts:
		...
		if opt in ["-S"]:
			skip_device_check=1
		if opt in ["-P", "--acrn_pwd="]:
			acrn_pwd=arg
		if opt in ["-u", "--acrn_user="]:
			acrn_user = arg
		if opt in ["-j", "--journal="]:
			journal = arg
	...
	while(1):
		try:
			...
			if line.find("ACRN:") != -1 and not isAcrnWrite:
				isAcrnWrite = True
				sd.write('sos_console 0\r'.encode())
			if line.find("login:") != -1 and not isCrashWrite:
				isCrashWrite = True
				sd.write('{0}\r'.encode().format(acrn_user))
			if line.find("Password:") != -1 and not isPwd:
				isPwd = True
				sd.write('{0}\r'.encode().format(acrn_pwd))
				sd.write('\r'.encode())
				# sd.write('{0}\r'.encode().format('systemctl start acrnd'))
				time.sleep(10)
				# sd.write('systemctl mask systemd-timesyncd\r'.encode())
				sd.write('\r'.encode())
				sd.write('\r'.encode())
				sd.write('{0}\r'.encode().format('journalctl -o short-precise > journal.log'))
				time.sleep(2)
				sd.write('\r'.encode())
				sd.write('\r'.encode())
				# sd.write('{0}\r'.encode().format("cat journal.log |grep -E 'Login|wlan|promiscuous|forwarding'"))
				# time.sleep(2)
				# sd.write('\r'.encode())
				# sd.write('{0}\r'.encode().format("cat journal.log |grep -E 'Kernel|vector|entry'"))
				# time.sleep(2)
				# sd.write('\r'.encode())
				# sd.write('{0}\r'.encode().format("cat journal.log |grep -E 'images|TPM|welcome'"))
				# time.sleep(2)
				# sd.write('\r'.encode())
				sd.write('{0}\r'.encode().format("ip addr show|grep inet"))
				time.sleep(2)
				sd.write('\r'.encode())
				sd.write('\r'.encode())
				# sd.write('{0}\r'.encode().format("echo 'PermitRootLogin Yes' > sshd_config"))
				# time.sleep(2)
				# sd.write('\r'.encode())
				# sd.write('\r'.encode())
				# sd.write('\r'.encode())
				...
				# 	sd.write('{0}\r'.format('yes'))
				# 	print '================+++=================='
				# print '===============-----=================='
				# if line.find("password")!=-1:
				# 	print "=============@@@@@@@============="
				# 	sd.write('{0}\r'.encode().format('123456'))
				# 	time.sleep(2)
				# print "=============##########============="
			# if 'password' in line:
			# 	print "=============1============="
			# 	sd.write('{0}\r'.encode().format('123456'))
			# 	print "=============1============="
			# 	sd.write('sh /usr/share/acrn/samples/apl-mrb/launch_uos.sh -V 2\r'.encode())
			# if line.find("sh: ./launch_UOS.sh: No such file or directory") != -1 and not isLaunch:
			# 	isLaunch = True
			# 	sd.write('sh /usr/share/acrn/samples/apl-mrb/launch_uos.sh -V 2\r'.encode())
