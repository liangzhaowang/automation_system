...
class watt_operate:
    ...
    def rewrite_watt_result(self, path, watttime_nodefined = True):
        ...
        try:
            ...
            with open(path, 'w') as wwfp:
                logger.info("rewrite %s !!!"%path)
                wwfp.writelines(watt_lines)
            return True
        except Exception,e:
            logger.error("watt text open %s error!!!"%path)
            logger.error(e)
    ...
    def watt_result_treat(self,ABS=30):
        ...
        logger.info("copy /data/watt.txt is ok")
    ...
    def get_watt_average(self):
        index_nu = 0
        if not self.watt_flags:
            return False
        try:
            with open(self.out_put_filename) as outfp:
                lines = outfp.readlines()
        except Exception, e:
            logger.info("open %s error :%s"%(self.out_put_filename,e))
            return False
        for line in lines:
        ...
        if index_nu > 0:
            ...
            logger.info("cpu_watt_val is %s !!!"%cpu_watt_val)
            return cpu_watt_val
        return False
